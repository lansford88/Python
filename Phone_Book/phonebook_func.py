...
def center_window(self, w, h):
        #MATHMATICS TO CENTER THE WINDOW                                        # pass in the tkinter frame (master) reference and the w and h
        # get user's screen width and height
        screen_width = self.master.winfo_screenwidth()                          #self.master is tkinters primary window,  (.winfo_screenwidth()) is a tkinter method you get the users width*******then we store it in our own variable name
        screen_height = self.master.winfo_screenheight()                        #self.master is tkinters primary window,  (.winfo_screenheight()) is a tkinter method you get the users height*******then we store it in our own variable name
        #calculate x and y coordinates to paint the app centered on the user's screen
        x = int((screen_width/2) - (w/2))                                       #in MAIN we made phonebook_func.center_window and set our width and hieght.
        y = int((screen_height/2) - (w/2))
        centerGeo = self.master.geometry('{}x{}+{}+{}'.format(w, h, x, y))       #.geometry gives dimensions to the form
        return centerGeo
...
def ask_quit(self):
    if messagebox.askokcancel("Exit Program", "Okay to exit application?"):
        #This closes the app
        self.master.destroy()                                                   #self.master(our main window).destroy() closes it out
        os._exit(0)                                                             #this line will completely delete refrences to memory

        

#*****************************************
        #CREATE DATABASE VERY FIRST TIME
def create_db(self):
    conn = sqlite3.connect('phonebook.db')                  #sqlite3 is the syntax, it's going to .connect to the ('database name') if it's not there it will create it           
    with conn:                                              #if the connection was successful, do the next line.
        cur = conn.cursor()                                 #cursor makes the changes in the object which is (.conn)
        ...
        #You have to commit() to save changes and close the db connection
        conn.commit()               #this commits and creates
    conn.close()
    first_run(self)                 #since this is the FIRST TIME running it, you need to create the first_run()       



def first_run(self):
    data = ('John','Doe','John Doe','111-111-1111','jdoes@email.com')           #tuple 
    conn = sqlite3.connect('phonebook.db')                                      #connects to the db
    with conn:
        cur = conn.cursor()                     #access cursor object, once you call it pass it to the variable called cur
        cur,count = count_records(cur)          #you have to pass the permissions with count_records(cur)   
        if count < 1:
...
def count_records(cur):
    ...
    count = cur.fetchone()[0]               #we need to extract it, and to get the first section of a tuple, you have to call the first index
    return cur,count
...
def onSelect(self,event):
    ...
    varList = even.widget                   #event.widet is whatever triggers the event
    select = varList.curselection()[0]      #this catches the index that they are clicking on this will give us the index, but not the text, 
    value = varList.get(select)             #we then pass the (select) into the varlist.get this will GET the text and put it in the variable (value)
    conn = sqlite3.connect('phonebook.db')
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT col_fname,col_lname,col_phone,col_email FROM tbl_phonebook WHERE col_fullname =(?)""", [value])
        varBody = cursor.fetchall()
        # This returns a tuple and we can slice it into 4 parts using data[] durint the iteration
        for data in varBody:
...
def addToList(self):
    ...
    var_fname = var_fname.title()       #The title says now that they removed the spaces, this will create a Cap at the start
    var_fname = var_lname.title()
    var_fullname =("{} {}".format(var_fname,var_lname)) #combine our normalzed names into a fullname
...
def onDelete(self):
    var_select = self.lstList1.get(self.lstList1.curselection()) #Listbox's select value(whatever they clicked on we are storing it in var_select)
    conn = sqlite3.connect('phonebook.db')
    with conn:
        cur = conn.cursor()
        #Check the count to ensure that this is not the last record in
        #the database... cannot delete last record or we will get an error
        cur.execute("""SELECT COUNT(*) FROM tbl_phonebook""")
        count = cur.fetchone()[0]
        if count > 1:
            confirm = messagebox.askokcancel("Delete Confirmation", "All information associated with, ({}) \nwill be permenantly delted from the database. \n\nProceed with the deletion request?".format(var_select))
            if confirm:
                conn = sqlite3.connect('phonebook.db')
                with conn:
                    cursor = conn.cursor()
...
###                 onRefresh(self) #update the listbox of the changes
            ...
            confirm = messagebox.showerror("Last Record Error", "({}) is the last record in the database and cannot be deleted at this time. \n\nPlease add another record first befor you can delete({}).".format(var_select,var_select))
    conn.close()
